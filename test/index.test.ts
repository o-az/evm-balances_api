...
describe('BalancesOf', async () => {
	const [walletAddress] =
		targetChain() === 'foundry' ? await walletClient('foundry').getAddresses() : ([testWalletAddress] as [Address])
	it('should return ETH balance if array empty', async () => {
		...
		const balanceFromClient = await publicClient(targetChain()).getBalance({ address: walletAddress })
